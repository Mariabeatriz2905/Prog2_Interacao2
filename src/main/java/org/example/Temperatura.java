...
public class Temperatura implements SinaisVitais{
     ...
     * @param sinaisVitais é convertido para o tipo específico temperatura
     */
    @Override
    public void adicionar(SinaisVitais sinaisVitais) {
        Temperatura temperatura = ((Temperatura) sinaisVitais);
        for (double m:temperatura.getMedicao()){
            this.medicao.add(m);
        }
        for (Data m:temperatura.getDataColheita()){
            this.dataColheita.add(m);
        }
        for (TecnicoDeSaude m:temperatura.getTecnicoResponsavel()){
